...
| Keep the review loop going after completion. | `index.html` adds `Remix deck`, `app.js` remixes the filtered local deck, `state.js` supplies thumbnail or first-line variants, and Playwright verifies old reviews remain, item IDs stay unique, variant metadata exports, and JSONL rows grow only after the next swipe. | Met locally |
