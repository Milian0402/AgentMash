...
| Keep the visible launch-prep build version current. | `package.json` and `package-lock.json` are at `0.3.0`; `index.html`, `support.html`, `privacy.html`, and `terms.html` show `v0.3.0`; `sw.js` cache is `agentmash-v43`; `scripts/launch-check.mjs` verifies package-version exposure and cache currency. | Met locally |
