...
- Public build output includes the machine-readable JSON Schema, OpenAPI, and MCP contract files future integrators need.
...
| Prepare for eventual backend/API/MCP intake without building it yet. | `schemas/intake.v1.json` documents `agentmash.intake.v1`; `schemas/api.v1.openapi.json` documents contract-only future backend routes; `schemas/mcp-tools.v1.json` documents contract-only future MCP tools; `index.html` exposes `Import drop` and review context fields; `app.js` validates and imports local agent-drop JSON without network calls; `packet.js` carries review context into feedback packets and eval rows. | Met locally |
...
| Prepare for a future backend/API/MCP path without building it. | `schemas/api.v1.openapi.json` defines contract-only future routes for intake, feedback bundle retrieval, and deletion; `schemas/mcp-tools.v1.json` defines contract-only MCP tools for submit, feedback retrieval, and deletion requests; `store/backend-api-mcp-handoff.md` lists implementation order and user-owned setup actions; `scripts/launch-check.mjs` and `scripts/verify-public-url.mjs` verify these contracts while the runtime remains network-free. | Met locally |
