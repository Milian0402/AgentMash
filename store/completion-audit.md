...
- Pending upload replacement clears old IndexedDB draft image bytes before storing the new pending upload.
...
| Avoid orphaned pending image uploads. | `state.js` exposes `deleteImageData()`, `app.js` clears the previous pending image before storing a replacement, and Playwright verifies changing from `first.png` to `second.png` leaves only one IndexedDB image key and submits the replacement key. | Met locally |
...
| Add Playwright regression coverage. | `tests/review-flow.spec.mjs` is wired through `npm run check`, uses `npm run serve` for native modules, and covers Nice, Undo, Nope, v2 packet shape, Keepers completion state, Remix repeat sessions with variant metadata, Endless auto-looping, Pairwise comparison export, empty Export workspace state, IndexedDB image persistence, pending upload replacement cleanup, profile image export/import, offline app-shell loading, and the 500-item stress path. | Met locally |
