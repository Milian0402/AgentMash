...
| Prepare for eventual backend/API/MCP intake without building it yet. | `schemas/intake.v1.json` documents `agentmash.intake.v1`; `index.html` exposes `Import drop` and review context fields; `app.js` validates and imports local agent-drop JSON without network calls; `packet.js` carries review context into feedback packets and eval rows. | Met locally |
...
| Add an intake validation contract. | `schemas/intake.v1.json` describes the local agent-drop payload and future intake shape, including source metadata, review context, return mode, artifacts, and raster image data. `app.js` rejects malformed drops with a clear status message, and `scripts/launch-check.mjs` verifies the file is present and keeps the import path local-only. | Met locally |
