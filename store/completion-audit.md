...
- Deck completion can start a local Remix session without overwriting existing export rows.
- `npm run check` includes Playwright e2e coverage for review flow, packet shape, Keepers completion state, Remix repeat sessions, empty Export workspace state, and image persistence.
...
| Keep the review loop going after completion. | `index.html` adds `Remix deck`, `app.js` clones the filtered local deck into new item IDs, and Playwright verifies old reviews remain, item IDs stay unique, and JSONL rows grow only after the next swipe. | Met locally |
...
| Add Playwright regression coverage. | `tests/review-flow.spec.mjs` is wired through `npm run check` and covers Nice, Undo, Nope, v2 packet shape, Keepers completion state, Remix repeat sessions, empty Export workspace state, and IndexedDB image persistence. | Met locally |
