...
- `npm run check` includes Playwright e2e coverage for review flow, packet shape, Keepers completion state, Remix repeat sessions with variant metadata, empty Export workspace state, and image persistence.
...
| Add Playwright regression coverage. | `tests/review-flow.spec.mjs` is wired through `npm run check` and covers Nice, Undo, Nope, v2 packet shape, Keepers completion state, Remix repeat sessions with variant metadata, empty Export workspace state, and IndexedDB image persistence. | Met locally |
...
- Fully automatic deck looping and pairwise mode remain product polish work not yet implemented.
