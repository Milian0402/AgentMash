...
- Export workspace can copy or download a self-contained feedback bundle for future API/MCP consumers, combining ready packets, eval rows, pairwise rows, run IDs, and summary counts.
- Pairwise-only preference data counts as ready export signal data in metrics, signal lists, artifact judged counts, and bundle summaries.
...
| Prepare for eventual backend/API/MCP intake without building it yet. | `schemas/intake.v1.json` documents `agentmash.intake.v1`; `schemas/api.v1.openapi.json` documents contract-only future backend routes; `schemas/mcp-tools.v1.json` documents contract-only future MCP tools; `schemas/examples/` gives future implementers sample intake, acknowledgement, and feedback bundle payloads; `index.html` exposes `Import drop`, review context fields, and feedback bundle export actions; `app.js` validates local agent-drop JSON and downloads local bundles without network calls; `packet.js` carries review context into feedback packets/eval rows and builds `agentmash.feedback-bundle.v1`. | Met locally |
...
| Keep starter-card quality consistent after the first swipe. | `render.js` and `styles.css` now render logo, copy, and product starter previews with brand-card, post-card, and product-render details; `scripts/launch-check.mjs` verifies those preview structures. | Met locally |
...
| Keep the visible launch-prep build version current. | `package.json` and `package-lock.json` are at `0.3.0`; `index.html`, `support.html`, `privacy.html`, and `terms.html` show `v0.3.0`; `sw.js` cache is `agentmash-v46`; `scripts/launch-check.mjs` verifies package-version exposure and cache currency. | Met locally |
